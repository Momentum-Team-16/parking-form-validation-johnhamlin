...
function validateCardNumber(number) {
  const regex = /^[0-9]{16}$/;
  return regex.test(number) ? luhnCheck(number) : false;
}
...
function validateForm(e) {
  ...
  const todayMonth = today.getMonth() + 1; // Convert zero-based month
  ...
  const checkExpDate = function (exp) {
    ...
    return (
      ...
      (todayShortYear === expYear && todayMonth <= expMonth)
    );
  };
